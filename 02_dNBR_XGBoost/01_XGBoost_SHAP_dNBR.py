...
功能：按Biome及保护区分组执行10折XGBoost回归和SHAP分析，并记录各阶段运行时间。
"""
...
def analyze_biome(
    ...
    """完成一个Biome×保护区组的交叉验证、SHAP、精度计算和保存。"""

    biome_start = time.perf_counter()
    print(f"\n[Biome {biome} | {group_name}] 开始处理", flush=True)
    biome_data = full_data.loc[
        (full_data["Biome"] == biome) & (full_data["PAs"] == pa_flag)
    ].copy()

    if biome_data.empty:
        elapsed = float(time.perf_counter() - biome_start)
        print(f"[Biome {biome} | {group_name}] 无数据，跳过；耗时 {format_duration(elapsed)}", flush=True)
        return None, {"Stage": f"Biome_{biome}_{group_name}", "Status": "Skipped_No_Data", "Total_Seconds": elapsed}
    ...
    if removed_rows > 0:
        print(f"[Biome {biome} | {group_name}] 删除缺失值记录：{removed_rows}", flush=True)
    if biome_data.empty:
        elapsed = float(time.perf_counter() - biome_start)
        print(f"[Biome {biome} | {group_name}] 删除缺失值后无数据，跳过", flush=True)
        return None, {"Stage": f"Biome_{biome}_{group_name}", "Status": "Skipped_After_DropNA", "Total_Seconds": elapsed}
    if len(biome_data) < cross_validation_splits:
        raise ValueError(
            f"Biome {biome} 的 {group_name} 组只有 {len(biome_data)} 条有效记录，"
    ...
    for fold_number, (train_indices, test_indices) in enumerate(kfold.split(input_data), start=1):
        ...
        print(
            f"[Biome {biome} | {group_name}] Fold {fold_number:02d}/{cross_validation_splits}: "
    ...
    save_seconds = save_biome_outputs(
        biome_directory=output_root / f"Biome_{biome}" / group_name,
    ...
    runtime_entry = {
        "Stage": f"Biome_{biome}_{group_name}",
        "Status": "Completed",
    ...
    print(
        f"[Biome {biome} | {group_name}] 完成：样本 {len(observed)}，"
...
def run_analysis(
    ...
    output_root: Path,
    biomes: list[int],
    group_definitions: list[tuple[int, str]],
    input_columns: list[str],
    output_column: str,
    cross_validation_splits: int,
    cross_validation_seed: int,
    ...
    for biome in biomes:
        for pa_flag, group_name in group_definitions:
    ...
    metrics_summary = pd.DataFrame(metric_rows).sort_values(["Biome", "Group"])
...
def main() -> None:
    ...
    input_excel = script_directory / "dNBR_datasets.xlsx"
    output_root = script_directory / "Figure_results"
    biomes = [1, 2, 3, 4, 5, 6, 12]
    group_definitions = [(1, "PAs"), (0, "nonPAs")]
    input_columns = ["AGB", "NDVI", "LFMC", "VPD", "Sc", "Rd"]
    output_column = "dNBR"
